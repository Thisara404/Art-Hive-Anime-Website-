      ...
      <div className="container flex h-16 items-center">
        <div className="mr-4 hidden md:flex">
          <Link to="/" className="mr-6 flex items-center space-x-2">
            <span className="font-bold text-xl bg-gradient-to-r from-ct-blue-500 to-ct-teal-500 bg-clip-text text-transparent">Art Hive</span>
          </Link>
          <nav className="flex items-center gap-6 text-sm">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                {CATEGORIES.map(category => <Link key={category.id} to={`/category/${category.slug}`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                    {category.name}
                  </Link>)}
              </div>
            </div>
